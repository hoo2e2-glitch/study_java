...
//	2. 사용자가 입력하는 값 중 정수 분리하기
	String getIntegerString(String input) {
		...
		for(int i = 0; i < input.length(); i++) {
			int c = input.charAt(i) - 48; // 문자 -> 숫자로 변환
			if(c >= 1 && c <= 9) { // 1~9사이일 때만
				result += (char)(c + 48); // 다시 문자로 변환해서 이어붙임
			}
		}
		return result;
	}
...
//		플레티넘 ~ 다이아
//		1. 두 정수를 전달하면 몫과 나머지를 배열로 전달하는 메서드
//		int[] datas = new MethodTask04().getDivide(10, 20);
//		for(int data : datas) {
