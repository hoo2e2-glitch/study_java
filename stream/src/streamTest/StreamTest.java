...
//		
//		   
//		   IntStream.rangeClosed(1, 50).filter(n -> n % 2 == 0).forEach(n -> {numbers2.add(n);System.out.println(n);});
//		
//		   IntStream.rangeClosed(1, 50).forEach((num) -> { if (num % 2 == 0) {
//               numbers2.add(num);
//		   }
//               
//           });
...
//		   IntStream.rangeClosed(1, 10).forEach((num) -> { numbers3.add(Integer.valueOf(num));});
//		   
...
//	      IntStream(data.split("")).map(String::toLowerCase).forEach(System.out::print); 
	      ...
	      IntStream.range(0, data.length())                    // 0, 1, 2, 3, 4, 5, 6
	         .forEach(i -> System.out.print( Character.toLowerCase(data.charAt(i)) ));  // 각 문자를 소문자로
	      
	      Arrays.stream(data.split("")).map(String::toLowerCase).forEach(System.out::print); 
	      
	      IntStream.range(0, data.length())
	         .map(i -> data.charAt(i) + 32)        // map에서 32 더하기 (소문자로 변환)
	         .forEach(i -> System.out.print((char)i)); // char로 캐스팅해서 출력
